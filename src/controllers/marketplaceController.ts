...
export const initiateMarketplacePurchase = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return next(
        new AppError("Invalid marketplace item ID", StatusCodes.BAD_REQUEST)
      );
    }

    if (quantity <= 0) {
      return next(
        new AppError("Quantity must be greater than 0", StatusCodes.BAD_REQUEST)
      );
    }

    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(
        () => reject(new Error("Purchase operation timed out")),
        30000
      );
    });
    ...
    try {
      const purchasePromise = async () => {
        ...
        if (!marketplaceItem) {
          return next(
            new AppError("Marketplace item not found", StatusCodes.NOT_FOUND)
          );
        }
        ...
        if (!marketplaceItem.isQuantityAvailable(quantity)) {
          return next(
            new AppError(
              `Insufficient stock. Only ${marketplaceItem.quantity} items available`,
              StatusCodes.BAD_REQUEST
            )
          );
        }
        ...
        const unitPrice =
          marketplaceItem.discountedPrice || marketplaceItem.price;
        ...
        if (!buyerWallet) {
          return next(
            new AppError("Buyer wallet not found", StatusCodes.NOT_FOUND)
          );
        }
        ...
        const systemUser = await User.findOne({
          email: "esthington@gmail.com",
        });
        if (!systemUser) {
          return next(
            new AppError("System user not found", StatusCodes.NOT_FOUND)
          );
        }
        ...
        try {
          const reference = `MKT-${id}-${Date.now()}-${uuidv4().substring(
            0,
            8
          )}`;
          ...
          marketplaceItem.soldQuantity =
            (marketplaceItem.soldQuantity || 0) + quantity;
          marketplaceItem.quantity = Math.max(
            0,
            marketplaceItem.quantity - quantity
          );
          ...
          console.log(
            "[NOTIFICATION] Creating purchase and sale notifications..."
          );
          const notificationPromises = [
            notificationService.createNotification(
              ...
              } (Qty: ${quantity}) for ₦${totalAmount.toLocaleString()} has been completed successfully.`,
            ...
            notificationService.createNotification(
              ...
              `New sale of ₦${totalAmount.toLocaleString()} received from ${
                buyer.firstName
              } ${buyer.lastName} for ${
                marketplaceItem.title
              } (Qty: ${quantity}).`,
          ...
          return res.status(StatusCodes.CREATED).json({
            success: true,
            message:
              "Purchase completed successfully! Your payment has been processed.",
            data: {
      ...
      if (
        ...
        error.message === "Purchase operation timed out"
      ) {
        return next(
          new AppError(
            "Purchase operation timed out. Please try again.",
...
export const deleteDocument = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    // Update listing
    listing.documents = listing.documents.filter(
...
// @access  Private
export const getUserMarketplacePurchases = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const userPurchases = listingsWithPurchases.map((listing) => {
      ...
      const userPurchasesForListing = listing.purchases.filter(
        (purchase: any) =>
          purchase.buyerId && purchase.buyerId.toString() === userId.toString()
      );
