...
export const protect = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!user.isActive) {
      return next(new AppError("User account is deactivated", StatusCodes.FORBIDDEN))
    }
...
export const isVerified = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!req.user.isEmailVerified) {
    return next(new AppError("Please verify your email first", StatusCodes.FORBIDDEN))
  }
